...
def extract_financial_entities(text):
    ...
    patterns = {
        'ORG': [
            r'\b[A-Z][a-zA-Z\s&]+(?:Ltd\.|Limited|Pvt\.|Private|Corporation|Company|Co\.|Group|Holdings|Technologies|Tech|Solutions|Industries|Enterprises)\b',
            r'\b(?:BSE|NSE):[A-Z]{1,10}\b',
            r'\b(?:NIFTY|SENSEX)(?:\s*50)?\b',
    ...
    unique_entities = {f"{e['text'].lower()}|{e['entity']}": e for e in entities}
    return sorted(unique_entities.values(), key=lambda x: x['score'], reverse=True)
